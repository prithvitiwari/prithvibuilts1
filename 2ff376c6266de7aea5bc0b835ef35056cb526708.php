...
<footer class="footer">
    <div class="container">
        <div class="row align-items-center flex-row-reverse">
            <div class="col-md-12 col-sm-12 text-center"> Copyright © 2022 <a href="#"></a>. Powered by <a href="#"> A2Z Providers</a>. All rights reserved. </div>
        </div>
    </div>
</footer> <!-- FOOTER END -->
<?php
...
$auth_user_order = $auth_user->order_count;
?>
</div>
<!-- BACK-TO-TOP -->
...
<script>
    setTimeout(function() {
        ...
        $('.success-alert .alert.alert-success').remove();
        $('.alert.alert-warning').remove();
    }, 5000);
    let auth_user_order = "<?php echo e($auth_user_order); ?>";
    let auth_user_buy_package = "<?php echo e($auth_user->buy_package); ?>";
    if ("<?php echo e(Route::Is('user.package')); ?>" || "<?php echo e(Route::Is('user.checkout')); ?>") {} else {
        if (auth_user_buy_package == 'false') {
            $('body').removeClass('timer-alert');
            swal({
                    title: "Warning",
                    text: "Right now, you do not have an active package buy at least 1 package.",
                    type: "warning",
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        window.location.href = "<?php echo e(route('user.package')); ?>";
                    }
                });
        }
    }
    ...
    function appendMessage() {
        setTimeout(function() {
            $('.success-alert div').fadeOut('slow');
        }, 5000);
        $('.success-alert').html('<div class="alert alert-success alert-block" style="margin: 1.5rem 0rem 1.5rem;">Referal code copy successfully.</div>');
    }

    function printIdCards() {
        let divToPrint = document.getElementById("divToPrint");

        let newWin = window.open("", "Visiting Card");
...
<script>
    $(function() {
        $("#geeks").click(function() {
            html2canvas($("#visiting_card"), {
                onrendered: function(canvas) {
                    var ctx = canvas.getContext("2d");
                    // ctx.fillStyle = "linear-gradient(90deg, #A100FFFF 0%, #71C4FFFF 100%)";
                    // Create gradient
                    var grd = ctx.createLinearGradient(0, 0, 200, 0);
                    grd.addColorStop(0, "#A100FFFF");
                    grd.addColorStop(1, "#71C4FFFF");
...
<script>
    ...
    //     console.log('sdfj');
    //     localStorage.setItem("active", Date.now());
    //     setTimeout(function() {
...
</html><?php /**PATH /home2/vijaycku/public_html/resources/views/frontend/common/userfooter.blade.php ENDPATH**/ ?>